...
Clean LSTM implementations for time series tasks.
Two variants:
- LSTM2seq: sequence-to-sequence output (sequence prediction)
- LSTM2one: sequence-to-one output (classification)
"""
...
class LSTM2one(nn.Module):
    """
    LSTM sequence-to-one classification model.
    ...
    def __init__(self, 
        ...
        Args:
            ...
            pooling_method: one of ("last", "mean", "max", "attention")
        """
...
# 注意：请在配置中显式指定类名（如 LSTM2one 或 LSTM2seq），不再提供默认别名。
